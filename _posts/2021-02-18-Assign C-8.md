...
또한, 필자는 곱셈(*) 연산자보다 우선순위가 더 높은 연산자가 존재하는 상황을 가정해봤다.
...
필자의 의도는 width*height를 진행한 후에 전위 ++ 연산에 의하여 그 값이 1만큼 증가한 값을 원했다.

그러나 실제 계산은 다음과 같이 진행된다.

```c
++width*height
```

즉, width의 값이 먼저 1 증가한 상태에서 height와의 곱셈 연산이 진행되므로 필자가 의도했던 연산 결과와는 전혀 다른 결과값이 도출된다.

이러한 상황을 미연에 방지하기 위해 다음과 같이 전체적으로 매크로를 한번 더 괄호로 감싸줘야 한다.
